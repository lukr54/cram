...
/// Must match `CLSID_CRAM_SHELL` in `cram-shell`. Changing either without the other silently breaks
/// registration: Explorer would look up a class id that nothing serves.
const CLSID: &str = "{934088FE-F647-4D05-9A52-FDF56127F43C}";

/// The key name under `ContextMenuHandlers`. Also what an uninstall deletes, so it must not change.
...
pub fn install() -> Result<()> {
    ...
    println!("context-menu handler goes. Restart Explorer if it does not appear straight away.");
    Ok(())
}
...
pub fn shell_cmd(args: &[String]) -> Result<()> {
    match args.get(2).map(String::as_str) {
        Some("install") | Some("add") => install(),
        Some("uninstall") | Some("remove") => uninstall(),
