...
// Such frames carry out-of-band metadata (e.g. the seekable-format seek table); `zstd -d` skips them
// and keeps decoding. Their layout is: magic(4) | frame_size(4, LE) | frame_size bytes.
...
pub fn decode_stream(
    ...
    Ok(match codec {
        ...
        // MultiFrameZstd (not a bare StreamingDecoder) so concatenated zstd *frames* (`cat`, pzstd)
        // are all decoded, not just the first.
        StreamCodec::Zstd => {
            let dec = next_zstd_frame(FramedSource::new(inner))
                .map_err(|e| ArchiveError::Backend(format!("zstd: {e}")))?;
            Box::new(MultiFrameZstd { dec })
        }
